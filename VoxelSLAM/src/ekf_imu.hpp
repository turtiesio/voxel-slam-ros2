...
inline double imuToSec(const builtin_interfaces::msg::Time& stamp) {
  return static_cast<double>(stamp.sec) + static_cast<double>(stamp.nanosec) * 1e-9;
}

// Helper to set time from seconds
inline void fromSec(builtin_interfaces::msg::Time& stamp, double sec) {
...
public:
  ...
  void motion_blur(IMUST &xc, pcl::PointCloud<PointType> &pcl_in, deque<sensor_msgs::msg::Imu::SharedPtr> &imus)
  {
    ...
    for(auto it_imu=imus.begin(); it_imu!=imus.end()-1; it_imu++)
    {
      ...
      if(imuToSec(tail.header.stamp) < last_pcl_end_time) continue;
      ...
      double cur_time = imuToSec(head.header.stamp);
      if(cur_time < last_pcl_end_time)
        cur_time = last_pcl_end_time;
      dt = imuToSec(tail.header.stamp) - cur_time;
    ...
    double imu_end_time = imuToSec(imus.back()->header.stamp);
